...
func TestUpdateUser_RoleAndPassword(t *testing.T) {
	...
	mk := func(name string, admin bool) User {
		t.Helper()
		u, err := store.CreateUser(ctx, CreateUserParams{Username: name, PasswordHash: "x", Role: roleFor(admin)})
		if err != nil {
			t.Fatalf("create %s: %v", name, err)
		}
		return u
	}
	on, off := auth.RoleAdmin, auth.RoleReader
	a := mk("adm_a", true)
	bob := mk("bob", false)

	if _, err := store.UpdateUser(ctx, UpdateUserParams{ID: 999999, Role: &on}); !errors.Is(err, ErrNotFound) {
		t.Fatalf("missing user: got %v, want ErrNotFound", err)
	}
	if _, err := store.UpdateUser(ctx, UpdateUserParams{ID: a.ID, Role: &off}); !errors.Is(err, ErrLastAdmin) {
		t.Fatalf("demote last admin: got %v, want ErrLastAdmin", err)
	}
	u, err := store.UpdateUser(ctx, UpdateUserParams{ID: bob.ID, Role: &on})
	if err != nil || !u.IsAdmin() || u.PasswordHash != "x" {
		t.Fatalf("promote bob: %+v, %v", u, err)
	}
	hash := "y"
	u, err = store.UpdateUser(ctx, UpdateUserParams{ID: a.ID, Role: &off, PasswordHash: &hash})
	...
	if _, err := store.db.Exec(ctx, `UPDATE users SET password_hash = '', role = 'admin' WHERE id = $1`, ghost.ID); err != nil {
		t.Fatal(err)
	}
	if _, err := store.UpdateUser(ctx, UpdateUserParams{ID: bob.ID, Role: &off}); !errors.Is(err, ErrLastAdmin) {
	...
	for i := 0; i < 5; i++ {
		if _, err := store.UpdateUser(ctx, UpdateUserParams{ID: a.ID, Role: &on}); err != nil {
			t.Fatal(err)
		}
		var wg sync.WaitGroup
		errs := make([]error, 2)
		wg.Add(2)
		go func() { defer wg.Done(); _, errs[0] = store.UpdateUser(ctx, UpdateUserParams{ID: a.ID, Role: &off}) }()
		go func() {
			defer wg.Done()
			_, errs[1] = store.UpdateUser(ctx, UpdateUserParams{ID: bob.ID, Role: &off})
		}()
		wg.Wait()
		...
		if _, err := store.UpdateUser(ctx, UpdateUserParams{ID: bob.ID, Role: &on}); err != nil {
			t.Fatal(err)
		}
	}
}
