...
//
// SentCount, FailedCount, LastSentAt, LastFailedAt, LastError and LastErrorAt
// are persistent counters stored on the webhooks row itself (migration 0031):
// they are maintained by MarkWebhookLogSent/MarkWebhookLogFailed, survive the
// webhook_logs retention cleanup and are only cleared by ResetWebhookStats.
// Sent24h, QueueDueCount and RetryingCount describe the current queue and are
// derived from webhook_logs.
...
type AdminWebhookSummary struct {
	...
	Sent24h       int
	QueueDueTotal int
	// FailedTotal is the sum of persistent failed_total counters.
	FailedTotal int
}
...
type AdminWebhookStore interface {
	...
	// ResetWebhookStats clears the persistent delivery counters of a webhook
	// owned by userID (ErrNotFound otherwise). Delivery logs are untouched.
	ResetWebhookStats(ctx context.Context, userID, webhookID int64) error
}
...
SELECT
  ...
  COALESCE(SUM(failed_total), 0)::int
FROM webhooks
WHERE user_id = $1`
...
func (s *PostgresStore) ResetWebhookStats(ctx context.Context, userID, webhookID int64) error {
	cmd, err := s.db.Exec(ctx, `
UPDATE webhooks
SET sent_total = 0,
    failed_total = 0,
    ...
    last_error_at = NULL,
    stats_reset_at = now()
WHERE id = $1 AND user_id = $2`, webhookID, userID)
	if err != nil {
		return fmt.Errorf("reset webhook stats: %w", err)
	}
