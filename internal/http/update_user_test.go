...
func TestUpdateUser(t *testing.T) {
	...
	if rec := put(bobTok, root.ID, `{"is_admin":false}`); rec.Code != http.StatusForbidden {
	...
	if rec := put(rootTok, root.ID, `{"is_admin":false}`); rec.Code != http.StatusBadRequest {
		t.Fatalf("self-demote: %d %s", rec.Code, rec.Body.String())
	}
	if rec := put(rootTok, 999, `{"is_admin":true}`); rec.Code != http.StatusNotFound {
	...
	rec := put(rootTok, bob.ID, `{"is_admin":true}`)
	...
	if rec := put(bobTok, root.ID, `{"is_admin":false}`); rec.Code != http.StatusOK {
		t.Fatalf("bob demotes root: %d %s", rec.Code, rec.Body.String())
	}
	if rec := put(rootTok, bob.ID, `{"is_admin":false}`); rec.Code != http.StatusForbidden {
		t.Fatalf("demoted root must lose access: %d", rec.Code)
	}
	if rec := put(bobTok, root.ID, `{"is_admin":true}`); rec.Code != http.StatusOK {
		t.Fatalf("re-promote root: %d %s", rec.Code, rec.Body.String())
	}
	if rec := put(rootTok, bob.ID, `{"is_admin":false}`); rec.Code != http.StatusOK {
	...
	if rec := put(tok1, root.ID, `{"is_admin":false}`); rec.Code != http.StatusConflict {
