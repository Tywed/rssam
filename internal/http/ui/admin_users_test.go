...
func TestUI_AdminUserRoleAndPassword(t *testing.T) {
	...
	if rec := postForm(t, mux, sid, "/ui/admin/users/7/role", url.Values{"role": {"admin"}}); rec.Code != http.StatusForbidden {
		t.Fatalf("role without csrf: %d", rec.Code)
	}
	if rec := postForm(t, mux, sid, "/ui/admin/users/1/role", url.Values{"csrf_token": {token}, "role": {"reader"}}); rec.Code != http.StatusBadRequest {
		t.Fatalf("self-demote: %d %s", rec.Code, rec.Body.String())
	}
	if rec := postForm(t, mux, sid, "/ui/admin/users/7/role", url.Values{"csrf_token": {token}, "role": {"root"}}); rec.Code != http.StatusBadRequest {
		t.Fatalf("invalid role: %d %s", rec.Code, rec.Body.String())
	}
	if rec := postForm(t, mux, sid, "/ui/admin/users/7/role", url.Values{"csrf_token": {token}, "role": {"admin"}}); rec.Code != http.StatusFound {
	...
	if len(users.updates) != 2 || users.updates[0].ID != 7 || users.updates[0].Role == nil || *users.updates[0].Role != auth.RoleAdmin || users.updates[0].PasswordHash != nil {
		t.Fatalf("role update params = %+v", users.updates)
	}
	if p := users.updates[1]; p.ID != 7 || p.Role != nil || p.PasswordHash == nil || !auth.CheckPassword(*p.PasswordHash, "longenough1") {
	...
	if d := auditStore.rows[0].Details; d["role"] != auth.RoleAdmin {
	...
	if rec := postForm(t, mux, sid, "/ui/admin/users/7/role", url.Values{"csrf_token": {token}, "role": {"reader"}}); rec.Code != http.StatusConflict {
		t.Fatalf("demote last admin: %d", rec.Code)
	}
	users.updateErr = storage.ErrNotFound
	if rec := postForm(t, mux, sid, "/ui/admin/users/7/role", url.Values{"csrf_token": {token}, "role": {"reader"}}); rec.Code != http.StatusNotFound {
	...
	if rec := postForm(t, mux2, sid2, "/ui/admin/users/7/role", url.Values{"csrf_token": {auth.CSRFToken("csrf-test", sid2)}, "role": {"admin"}}); rec.Code != http.StatusForbidden {
