...
- Общий каталог лент: одну ленту читают несколько пользователей, опрашивается и хранится она один раз; у каждого читателя своя категория, вебхук, статусы и избранное. Подписка через «Каталог» или по URL (совпадающий адрес не создаёт дубликат); записи удаляются физически, когда их не осталось ни у одного читателя
- Роли: **admin** — всё; **editor** — создаёт и редактирует ленты каталога, фильтры, вебхуки, OPML; **reader** — подписывается, читает, ставит избранное, ведёт свои категории и метки. Лимиты на редактора — `MAX_FEEDS_PER_EDITOR`, `MAX_WEBHOOKS_PER_EDITOR`, `EDITOR_MIN_POLL_INTERVAL`
- Фильтры (regex RE2 и полнотекстовые запросы с морфологией) с действиями: метка, вебхук, удаление
- Вебхуки HTTP, Telegram и Max; после доставки — `on_success_entry`; режим дайджеста (одно сообщение раз в N минут) и системные уведомления (ленты остановлены/молчат, вебхук не доставляется, есть обновление, бэкап устарел). Протокол: [docs/WEBHOOK_RECEIVER_SPEC.md](docs/WEBHOOK_RECEIVER_SPEC.md)
...
- REST `/v1` (API-ключи), схема `GET /openapi.json` и справочник `GET /docs` (собственная страница, без внешних ресурсов)
...
- Рекомендуется `wal_compression = lz4` в PostgreSQL (`ALTER SYSTEM SET wal_compression = 'lz4'; SELECT pg_reload_conf();`): основной объём WAL у rssam — полные образы страниц при отметках «прочитано», сжатие сокращает его вдвое (замер: 11,8 → 5,8 КБ на запись) ценой ~1 % CPU. Настройка на весь кластер, поэтому installer только подсказывает, если она выключена.
...
`GITHUB_REPO=Tywed/rssam` — сравнение версии в UI с latest release (и уведомление об обновлении через вебхук с «системными уведомлениями»). `BACKUP_DIR`/`BACKUP_MAX_AGE` — каталог дампов `rssam-backup.sh` и допустимый возраст свежайшего (по умолчанию 36h) для уведомления «бэкап устарел».
